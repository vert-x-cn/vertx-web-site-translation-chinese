...
您还可以指定一个索引值，通过 `setXXX` 方法写入数据到 `Buffer` ，它也存在各种不同数据类型的方法。
...
可使用 `getUnsignedXXX` ,
`appendUnsignedXXX` 和 `setUnsignedXXX` 方法将无符号数从 `Buffer` 中读取或追加/设置到 `Buffer` 里。这对以优化网络协议和最小化带宽消耗为目的实现的编解码器是很有用的。
