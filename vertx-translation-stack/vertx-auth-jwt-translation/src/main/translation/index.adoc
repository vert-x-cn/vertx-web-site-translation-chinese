...
这个组件包含了一个开箱即用的JWT实现。 要使用这个项目，将下面的依赖添加到构建描述符里的`dependencies`部分。
...
在决定使用JWT之前，需要重点注意的是JWT并不加密payload，它仅对payload签名。
...
你可以使用 {@link io.vertx.ext.auth.jwt.JWTAuth} 来创建一个提供者的实例，并指定一个JSON对象的配置。
...
auth提供者需要在类路径或文件系统上加载一个keystore文件，keystore文件要么有一个
`https://docs.oracle.com/javase/8/docs/api/javax/crypto/Mac.html[javax.crypto.Mac]`
要么有一个 `https://docs.oracle.com/javase/8/docs/api/java/security/Signature.html[java.security.Signature]` 用于签名和验证生成的令牌。
